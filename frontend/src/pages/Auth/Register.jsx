...
const ClerkGoogleRegisterButton = ({ role, storeCategory, onError, onSuccess }) => {
  ...
  useEffect(() => {
    if (isSignedIn && clerkUser) {
      ...
      const payload = {
        ...
        role: role,
        retailerCategory: role === 'retailer' ? storeCategory : undefined,
      };
  ...
  const handleClerkSignup = async () => {
    ...
    try {
      await signUp.authenticateWithRedirect({
        ...
        redirectUrlComplete: `${window.location.origin}/register`,
      });
    } catch (err) {
      onError(err.errors?.[0]?.message || err.message || 'Google Sign-Up failed');
    }
  };
  ...
  return (
    <button
      ...
      disabled={syncing}
      className="w-full flex items-center justify-center gap-3 py-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all mb-4 group outline-none disabled:opacity-60"
    >
      ...
      <span className="font-inter text-sm font-semibold text-white">
        {syncing ? 'Signing up with Clerk...' : 'Sign up with Google'}
      </span>
    </button>
  );
};
          ...
          >
            ...
            {hasClerkKey ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all mb-4 group outline-none"
              >
                <svg className="w-5 h-5 group-hover:scale-110 transition-transform" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                ...
                <span className="font-inter text-sm font-semibold text-white">Sign up with Google</span>
              </button>
            )}
